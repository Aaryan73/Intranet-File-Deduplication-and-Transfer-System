...
import { FaFilePdf, FaFileWord, FaFileExcel, FaFileImage, FaFileAudio, FaFileVideo, FaFileArchive, FaFile, FaExclamationTriangle, FaPause, FaPlay, FaTimes, FaUser } from 'react-icons/fa';
  ...
  return (
    <div className="flex flex-col hide-scrollbar pb-10 px-1  gap-4 overflow-y-scroll">
      <div className='p-3 flex items-center justify-between'>
        <ToggleSwitch />
        <button onClick={() => { switchTab('profile'); }} className="px-6 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600 inline-flex items-center justify-center gap-2">
          <FaUser /> Profile
        </button>
      </div>
      ...
      <div className="space-y-4">
        ...
        {ongoingDownloads.length > 0 ? (
          ongoingDownloads.map((download) => (
            <div key={download.id} className="block mx-2 px-4 py-6 border bg-white border-gray-200 rounded-lg shadow">
              <div className='flex items-center justify-between'>
                ...
                <div className="flex gap-2">
                  {download.paused ? (
                    <button onClick={() => handleResume(download.id)} className="px-3 py-1 text-white bg-blue-500 rounded hover:bg-blue-600">
                      <FaPlay />
                    </button>
                  ) : (
                    <button onClick={() => handlePause(download.id)} className="px-3 py-1 text-white bg-yellow-500 rounded hover:bg-yellow-600">
                      <FaPause />
                    </button>
                  )}
                  <button onClick={() => handleCancel(download.id)} className="px-3 py-1 text-white bg-red-500 rounded hover:bg-red-600">
                    <FaTimes />
                  </button>
                </div>
              </div>
        ...
        {downloads.slice(0, 10).map((download) => (
          <div key={download.id} className={`block hover:border-blue-500 mx-2 px-4 py-4 border border-gray-200 rounded-lg shadow ${download.state === 'interrupted' ? 'bg-red-100 border-red-400' : 'bg-white'}`}>
